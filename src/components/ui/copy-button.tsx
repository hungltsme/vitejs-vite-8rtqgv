...
export function CopyButtonWithText({ value, className, src, variant = 'outline', ...props }: CopyButtonProps) {
  const [hasCopied, setHasCopied] = React.useState(false);
  ...
  return (
    <Button
      variant={variant}
      className={cn('relative z-10 hover:bg-zinc-700 hover:text-zinc-50 [&_svg]:h-4 [&_svg]:w-4', className)}
      onClick={() => {
        copyToClipboardWithMeta(value);
        setHasCopied(true);
      }}
